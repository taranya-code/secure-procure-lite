...
cmd_install_deps() {
  ...
  # Bootstrap pip via ensurepip — required on Python 3.12+ where venv
  # no longer guarantees a pip binary is placed in venv/bin/
  info "Bootstrapping pip..."
  "$VENV_DIR/bin/python3" -m ensurepip --upgrade --root-user-action=ignore 2>/dev/null || true

  info "Upgrading pip..."
  "$VENV_DIR/bin/python3" -m pip install -q --upgrade pip

  info "Installing packages from requirements.txt..."
  "$VENV_DIR/bin/python3" -m pip install -q -r "$SCRIPT_DIR/requirements.txt"
...
cmd_validate() {
  ...
  if [[ -f "$VENV_DIR/bin/python3" ]]; then
    if "$VENV_DIR/bin/python3" -c "import flask, flask_login, flask_wtf, sqlalchemy, werkzeug" 2>/dev/null; then
  ...
  if [[ -f "$VENV_DIR/bin/python3" ]]; then
    cd "$SCRIPT_DIR"
    local user_count
    user_count=$("$VENV_DIR/bin/python3" -c "
...
ensure_deps() {
  ...
  if [[ -f "$VENV_DIR/bin/python3" ]]; then
    "$VENV_DIR/bin/python3" -c "import flask, flask_login, flask_wtf, sqlalchemy" 2>/dev/null || needs_install=true
  else
    needs_install=true
  fi
